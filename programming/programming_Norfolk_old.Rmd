...
Let us call the ratio of meningitis/encephalitis (EncephMen) cases to the total number of cases the *neuroinvasive disease rate*. 
...
output <- c() # Question: What is this line doing? Answer: This line takes the output values designated by the function and forms them into a list called "output" which is then used for plotting. In this example the function multiples all values in the sequence by 10 creating a positive linear scatterplot.
for(t in times){
