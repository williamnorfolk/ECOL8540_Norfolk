...
#I assume severe disease rate = neuroinvasive disease rate so will calculate this for the given states using the function.
...
#subset the data by locaiton
ca_only <- subset(wnv_data, State == "California")
co_only <- subset(wnv_data, State == "Colorado")
ny_only <- subset(wnv_data, State == "New York")

mean_and_stderror(ca_only$neuro_rate)
#Average neuroinvasive rate in California was 0.5123430 with a standard error of 0.1113079 
mean_and_stderror(co_only$neuro_rate)
#Average neuroinvasive rate in Colorado was 0.22358004 with a standard error of 0.04218512 
mean_and_stderror(ny_only$neuro_rate)
...
#Probably a tidy-er way to do this, but getting a bit stuck on this application
#List all values of interest
state_list <- c("California", "Colorado", "New York")
mean_neuro_rate <- c(0.5123430, 0.22358004, 0.81486420)
standard_error <- c(0.1113079, 0.04218512, 0.03717168)
#recombine to new dataframe
three_states <- data.frame(state_list, mean_neuro_rate, standard_error)
#plot function outputs
ggplot(three_states, aes(state_list, mean_neuro_rate)) + geom_bar(stat = "identity") +
  geom_errorbar(aes(state_list, ymin = mean_neuro_rate - standard_error, ymax = mean_neuro_rate + standard_error)) 
...
#**Exercise. How does your choice of longitudinal breakpoint matter to the evidence for a geographic difference in case fatality rate? Combine conditional execution and looping to study the difference in case fatality rate over a range of breakpoints. What is the "best" longitude for separating case fatality rate in the East vs. West?**
 # _The selected breakpoint greatly influences the geographic differences in case fatality rate by including or excluding regions which may have a higher disease incidence and/or mortality rate due to confounding factors (vector presence, environmental health, sanitation, etc.). The inclusion or exclusions of "hot zone" geographic regions based on personal opinion of what is Eastern adn Western United States can skew the case fatality rate based on the individual opinion of the breakpoint setter. Based on the longitudinal setting it appears that -91.85 is the best breakpoint to divide th east and western US. Beyond these breakpoints the cfr tends to spike and dip as the longitude changes suggesting an influence on the cfr based on geographic shifting._

#Select a range of breakpoints. We will use Kansas, Mississippi, Ohio, Oklahoma, and Arkansas
breakpoints <- c(-82.66, -90.06, -91.89, -97.98, -98.21)

#loop over breakpoints to find mean case fatality rate by region
east <- c()
west <- c()
for(breaks in breakpoints){
  look.at <- wnv_data$Longitude == breaks #Look.at is a boolean vector that directs the outcome of each iteration
  if (wnv_data$Longitude <= wnv_data$Longitude[look.at]){
...
plot(breakpoints, west, type='p')
plot(breakpoints, east, type='p')
